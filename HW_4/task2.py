...
readed=open(r"C:\Users\\79816\Desktop\\file.txt" , "r")
garden=list(map(int,readed.read().split()))
max=garden[0]
for i in range(1, len(garden)-1):
    sum=garden[i-1]+garden[i]+garden[i+1]
    if max<sum:
        max=sum
sum=garden[-1]+garden[0]+garden[1]
if max<sum:
    max=sum
sum=garden[-2]+garden[-1]+garden[0]
if max<sum:
    max=sum
print(max)